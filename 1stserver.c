      ...
      if(nrb==0)
       {
        ...
        write(client,username,32);//write to the main server to clean the record
        strcat(closemsg,username);
        strcat(closemsg," left the room");
        printf("%s closed\n",username);
        for(i=0;i<=cid;i++)
          {
            if((clirec[i].online==0) || (clirec[i].id==nowid));
            else
            write(clirec[i].fd,closemsg,64);
            clirec[nowid].fd=0;
          }
...
int main(int argc ,char * argv[])
{
    ...
    strcpy(thisservrec.name,"netserver");//this chat room record
